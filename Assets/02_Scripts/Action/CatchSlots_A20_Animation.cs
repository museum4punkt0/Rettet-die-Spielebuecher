 ...
 * 
 *</summary>*/
public class CatchSlots_A20_Animation : MonoBehaviour
{
 ...
 */
  ...
  void Update () {
    ...
    if (Input.GetMouseButtonUp(0) && !IsPointerOverUIObject()) { // && MasterActionManager_BS.self.state.batteryCount > 0) {
          numOfTries++;
        ...
        if (!isMovingOne) {
          ...
          /* see if there is a Animator in beamMe and set its var to "catched"*/
          Animator animator = beamMe.GetComponent<Animator>();
          if (animator != null) {
             // Debug.Log("The Animation should start");
    ...
    if (isMovingOne) {
      // Debug.Log("isMovingOne " + beamMe.name);
      // do the animation
