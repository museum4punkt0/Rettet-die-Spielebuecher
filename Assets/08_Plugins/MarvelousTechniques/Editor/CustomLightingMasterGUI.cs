...
{	
	...
	static string USE_DIST_LIGHT_MULTI = "USE_DIST_LIGHT_MULTI";
	...
	bool distanceLightingMulti = false;
	...
	void initModes(Material m){
		...
		distanceLightingMulti = m.IsKeywordEnabled (USE_DIST_LIGHT_MULTI);
	...
	public override void OnGUI (MaterialEditor me, MaterialProperty[] props) {
		...
		if (distanceLightingMode) {
			...
			GUILayout.Space(15);
			distanceLightingMulti = GUI.Toggle (r, distanceLightingMulti, "Multi");
			if (distanceLightingMulti) {
				material.EnableKeyword (USE_DIST_LIGHT_MULTI);
			}
			else {
				material.DisableKeyword (USE_DIST_LIGHT_MULTI);
			}
			r.y += 18;
			GUILayout.Space(15);
