...
#if USE_DIR_LIGHT
uniform half3 IP(_LightDirF);
uniform half3 IP(_LightDirT);
uniform half3 IP(_LightDirR);
#endif
...
#ifdef USE_GRADIENT
	half gradient = saturate((ypos - IP(_GradientStartY)) / IP(_GradientHeight));
	f_color = lerp(IP(_FrontColorBottom),f_color,gradient);
	r_color = lerp(IP(_RightColorBottom),r_color,gradient);
	t_color = lerp(IP(_TopColorBottom),t_color,gradient);
	rimColor  = lerpIP((_RimColorBottom),rimColor,gradient);
#endif
...
#if USE_DIR_LIGHT
	half f_d = acos((dot(IP(_LightDirF),normal)))/1.5708;
	half r_d = acos((dot(IP(_LightDirR),normal)))/1.5708;
	half t_d = acos((dot(IP(_LightDirT),normal)))/1.5708;
...
CL_OUT_WPOS customLightingWPosVertSimple(CL_IN v, half3 mainColor,half3 rimColor, half rimPower,half rightLight, half frontLight, half topLight, half3 ambientColor, float ambientPower){
	...
	half lpos = o.wpos.y;
		...
		lpos = v.local_height.y;
	#endif
	half gradient = saturate((lpos - IP(_GradientStartY)) / IP(_GradientHeight));
...
fixed4 calculteDistanceLight(fixed4 currentLight,CL_OUT_WPOS v){
	half l =length(IP(_LightPos) - v.wpos);
 	l = clamp(min(l,(_LightMaxDistance))/IP(_LightMaxDistance),0,1);
